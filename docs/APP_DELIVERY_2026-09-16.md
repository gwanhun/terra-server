...
| 푸시 이벤트 발송 | ⏸️ **secret 수령 후 활성화**. `skipped` 는 앱 "발송 시작" 신호 후 별도 스위치로 |
...
| 3.3 | 푸시 이벤트 | ✅ 회신 반영 + `skipped` 준비 | **secret 전달**만 남음 | ★ |
...
**"skipped 발송 시작해도 됩니다" 신호를 주시면 스위치(`PUSH_EVENT_SKIPPED_ENABLED`)를 켭니다.** 그 전엔 422 로 거절된다고 하셔서 기본 꺼짐입니다.
...
`이광헌_전달_2026-09-16` 폴더로 초안 5개와 검증 스크립트를 받았습니다. 검토 후 별도 회신하겠습니다.
...
9/16 결정 회신으로 **7건 전부 해소**됐습니다. 남은 건 신호 둘뿐입니다.

| # | 필요한 것 | 그러면 |
|:---:|---|---|
| 1 | **`PUSH_EVENT_INGEST_SECRET`** (별도 채널) | 환경변수 2줄 + 브리지 재시작으로 즉시 발송 시작 |
| 2 | **"skipped 발송 시작해도 됩니다"** 신호 | 스위치 켜서 `device.action.skipped` 발송 |
...
| 개체↔카메라 이력 | 앱 팀 SQL 초안 5개 수령. **검토 진행** → 별도 회신 |
