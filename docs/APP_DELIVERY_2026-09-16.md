...
9/16 회신으로 7건이 해소됐습니다. 남은 것을 **막고 있는 정도** 순으로 적습니다.

| # | 필요한 것 | 없으면 막히는 것 | 관련 |
|:---:|---|---|---|
| 1 | **기록을 남기는 기기 해제 — 설계 합의** | 저희 우선순위 1번 작업이 시작도 못 합니다. 지금 삭제는 센서 기록·예약·영상까지 지웁니다 | §1.3 |
| 2 | **푸시 `PUSH_EVENT_INGEST_SECRET`** | 푸시 발송. 값만 주시면 그 즉시 켭니다 | §3.3 |
| 3 | **LED 타이머를 펌웨어에 넣을지** + 플래그 이름·상한(2h vs 3h) | 넣으면 펌웨어 과제, 안 넣으면 앱이 칩만 제거하면 끝 | §1.4 |
| 4 | **SQL 초안 5개 경로** | 저희가 읽지 못해 검토가 멈춰 있습니다 | §6 |
| 5 | 히터 예약을 서버에서 400 으로 막을지 | 막지 않아도 앱이 안 보여주면 되지만, 이중 안전장치가 됩니다 | §4.1 |
| 6 | `no_ack` 임계값 30초가 적절한지 | 저희 기본값입니다. 설정값이라 바꾸기 쉽습니다 | §3.3 |
| 7 | `device.action.skipped` 타입 정의 (2차) | 서버엔 이미 감사 기록이 쌓이고 있어 타입만 정하면 됩니다 | §3.3 |

**1번이 가장 급합니다.** 나머지는 병렬로 진행 가능하지만, 기기 해제는 합의 전까지 저희가 손댈 수 없습니다.
...
| 작업 | 상태 |
|---|---|
| 기록을 남기는 기기 해제 | **앱 합의 대기 — 우선순위 1** (§1.3 · §7-1) |
| 이름 중복 제약 + 409 | ✅ 완료 (§3.2) |
| 지표별 유효 표본 수 | ✅ 완료 (§3.1) |
