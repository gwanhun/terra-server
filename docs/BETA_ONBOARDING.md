...
4. 붙일 계정 행에서:
   ...
   - **사육장 연결**(2026-09-23~): 등록 시 그 계정의 사육장에 자동 연결한다. 사육장이 1개면 자동,
     2개 이상이면 이름 옆 셀렉트에서 고른 뒤 다시 누른다. **0개면 🔴 경고** — 등록은 되지만
     `enclosure_id` 가 NULL 이라 **앱 홈·카메라 탭에 안 보인다**(앱은 사육장 세트 단위로 묶는다).
     테스터가 앱에서 사육장을 먼저 만들게 하거나, 만든 뒤 기기 편집에서 연결하게 안내한다.
     운영자가 대신 사육장을 만들지는 않는다(petcam owner 결정).
...
- 각 항목 옆 **[테스트]** → 아래 줄이 펼쳐진다(다시 누르면 닫힘).
  ...
  - **🌡보정(℃)**: 온도 보정 오프셋. 측정값에 더할 값을 넣고 [적용] (예: 2℃ 높게 나오면 `-2`). 기기가 NVS 에 저장해 **재부팅·펌웨어 업데이트 후에도 유지**되고, 센서 읽기 직후 적용돼 **LCD·앱·웹이 모두 같은 값**이 된다. 범위 ±10℃.
...
- 각 항목 옆 **[해제]** → 소프트 해제. 목록에서만 사라지고 클립·라벨링 등 하위 데이터는 보존. **클립이 쌓인 기기·카메라는 이걸 쓴다.**
- 각 항목 옆 **[삭제]** → 하드 삭제(되돌릴 수 없음, mosquitto 계정도 제거). **클립이 있으면 참조 제약으로 500 이 난다** — `motion_clips` 를 참조하는 분석·라벨링 테이블이 45개라 CASCADE 로 뚫으면 정답(GT) 데이터까지 지워지므로 그렇게 하지 않는다. 하드 삭제는 **클립이 없는 갓 등록한 기기**의 중복 정리용으로만.
- **[전체 확인]** → 모든 계정 순차 조회 (로그인 rate limit 약 30회/5분 주의).
- **선택 일괄제어**: 각 기기 앞 체크박스로 여러 대(계정 섞여도 됨) 선택 후 상단 버튼(💧펌프·🌀팬·❄️냉각팬·💡LED·💦1/2/3초) 하나로 **동시 명령**. 계정별로 묶어 처리하고 ack 를 모아 `전송 N / ✅ / ⏳` 요약. **반복**: 체크 + 간격(초) 켜고 버튼 누르면 그 명령을 간격마다 반복, [정지]로 멈춤(스트레스/반복 테스트).
