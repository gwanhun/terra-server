...
- 각 항목 옆 **[테스트]** (기기) → 누르면 아래 줄이 펼쳐지며 **제어 버튼 전체**(💧펌프·🌀팬·❄️냉각팬·💡LED·🔥히터·⚠래치해제·💦분무 1/2/3초)가 뜬다. 그 계정 JWT 로 실제 명령을 보내고 각 명령의 ack 를 폴링해 `✅`(응답 OK)/`⏳`(무응답, 오프라인)/`✖`(거부)를 표시. 다시 누르면 닫힘. **프로덕션에서만 실제 전달**(브리지가 명령을 MQTT 로 전달).
