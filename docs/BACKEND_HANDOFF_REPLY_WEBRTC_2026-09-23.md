...
> **성격**: 요청 1은 **구현 완료**(DB 적용됨 / 코드는 재시작 대기), 요청 2는 **설계 확인 후 결정 대기**. 선택 3건 중 2건은 **이미 해결돼 있거나 진단이 어긋나 있습니다** — 그 근거를 코드 위치와 함께 적었습니다.
> **관련 커밋**: `f158db2` (main)
> **계약 정본**: [APP_WEBRTC.md](APP_WEBRTC.md) — 이번에 §4.2.1 추가
>
> ⚠️ **반영 단계가 둘입니다.** DB 테이블은 **2026-09-23 적용 완료**(바로 INSERT 가능)지만,
> `offer_attempts`/`answer_ms` 를 내려주는 **서버 코드는 운영 재시작 시점부터** 유효합니다.
> 재시작 전에는 두 필드가 응답에 없어 로그 컬럼이 NULL 로 남습니다 — 디버깅 낭비를 막기 위해
> 적어둡니다. 현재 상태는 §1.4 참고.
...
| 1+ | (서버 추가 제안) | ✅ 코드 반영 (`f158db2`) — `POST /webrtc/offer` 응답에 `offer_attempts`·`answer_ms`. **운영 반영은 재시작 시점** (§1.4) | **두 값을 로그 행에 넣어주세요** — 실패 원인 분리의 핵심 |
...
| 대상 | 상태 |
|---|---|
| DB 테이블 `webrtc_connect_logs` | ✅ **2026-09-23 적용 완료** — 지금 바로 INSERT 가능 |
| 서버 코드 (`offer_attempts`·`answer_ms`) | 커밋 `f158db2` main 반영. **운영 서버 재시작 시점부터 유효** |

**확인 방법**: 라이브를 한 번 연 뒤 `POST /webrtc/offer` 응답에 두 키가 있는지 보시면 됩니다.
없으면 아직 재시작 전입니다 — 그 동안 INSERT 하셔도 무방하고, 두 컬럼만 NULL 로 남습니다.
