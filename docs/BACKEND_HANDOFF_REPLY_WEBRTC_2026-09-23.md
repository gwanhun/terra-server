...
> **성격**: 요청 1은 **완료·배포**, 요청 2는 **별도 인스턴스로 결정 → 진행 중**. 선택 1(18초)은 첫 회신이 틀려 **§3 을 정정**했고, 선택 2 는 이미 방어돼 있습니다 — 근거를 코드 위치와 함께 적었습니다.
> **관련 커밋**: `f158db2` (main)
> **계약 정본**: [APP_WEBRTC.md](APP_WEBRTC.md) — 이번에 §4.2.1 추가
>
> ✅ **2026-09-23 운영 반영 완료** — DB 테이블 적용 + 서버 코드 배포·재시작 모두 끝났습니다.
> **지금 바로 INSERT 하시면 되고, `offer_attempts`/`answer_ms` 도 응답에 들어갑니다.** (§1.4)
...
| 대상 | 상태 |
|---|---|
| DB 테이블 `webrtc_connect_logs` | ✅ **2026-09-23 적용 완료** (cron jobid=10) |
| 서버 코드 (`offer_attempts`·`answer_ms`) | ✅ **2026-09-23 배포·재시작 완료** (`f158db2`) |

**양쪽 다 끝났으니 바로 붙이시면 됩니다.** 혹시 두 필드가 응답에 안 보이면 알려주세요 —
라이브를 한 번 열고 `POST /webrtc/offer` 응답에 두 키가 있는지가 가장 빠른 확인입니다.
...
## 3. 선택 1 — "새 세션 IDR 즉시 전송" — 정정 (2026-09-23 회신 반영)

> ⚠️ **이 절은 첫 회신에서 틀렸던 부분을 고친 것입니다.** 첫 회신은 "18초는 offer 재시도"라고
> 했는데, 앱팀 실측(answer 0.8초, ICE connected 1.3초, connected 이후 ~18초)이 맞습니다.
> offer 재시도는 answer 수신 **전**에 쌓이므로 connected 이후 구간을 설명하지 못합니다.
> 지적해 주셔서 감사합니다. 아래는 다시 조사한 결과입니다.

### 3.1 펌웨어는 이미 IDR 을 기다렸다 보냅니다 (변경 없음)
...
### 3.2 그 대기 비용은 최대 2.5초입니다 (첫 회신의 1.5초는 오기)

라이브는 카메라/클립 FPS(`CONFIG_APP_H264_FPS=10`)가 아니라 **라이브 전용 FPS** 를 씁니다.

```
CONFIG_APP_H264_GOP   = 15   (프레임 단위)
CONFIG_APP_WEBRTC_FPS = 6    (main.c 송출 루프의 pacing 기준)
→ IDR 주기 = 15 / 6 = 2.5초
```

첫 회신에서 10fps 로 계산해 1.5초라고 적은 것은 잘못이었습니다. **어느 쪽이든 18초는 설명하지 못합니다.**

### 3.3 진짜 원인 — H.264 락 starvation (2026-08-12 수정됨)

`main.c` 라이브 송출 루프의 주석에 실측 기록이 남아 있습니다:

> `[starvation 수정] trylock(0)+200ms 폴링은 매 프레임 lock 을 반환 직후 재선점하는
> motion_upload_task 에게 계속 지는 starvation 을 만들었다 (실측: peer state=7 후 38초간
> 송출 시작 실패). bounded blocking 으로 대기 …`

즉 **연결(state=7) 직후 H.264 인코더 락 경쟁에서 라이브 태스크가 굶어 최대 38초간 첫 프레임을
못 보내던 버그**입니다. 앱팀이 측정한 18초가 정확히 이 범위입니다.

| | 날짜 |
|---|---|
| 앱팀 실측 (`[webrtc-timing]`, connected 이후 ~18초) | 2026-07-19 |
| starvation 수정 (`trylock` → bounded blocking 1000ms) | **2026-08-12** (`d019762`) |

**7월 측정값은 8월에 고쳐진 버그를 본 것입니다.** "그 이후 해소됐을 수 있다"는 앱팀 추측이
맞았고, 이유가 GOP 가 아니라 락 경쟁이었을 뿐입니다.

### 3.4 결론 — 로그 판정 기준값

앱팀 제안대로 `ms_first_frame − ms_connected` 로 판정하면 됩니다. **기준값은 2.5초 근처**입니다
(IDR 대기 상한). 이보다 확연히 크게 나오는 세션이 있으면 그때 force-IDR 이나 락 우선순위를
다시 봅니다. 지금 펌웨어에서는 그럴 이유가 없다고 보고 있습니다.

> `offer_attempts` 는 여전히 유효합니다 — 카메라 무응답 재시도를 재는 값이고, 18초와
> 무관할 뿐입니다. §1.3 은 그대로입니다.
...
→ ✅ **2026-09-23 동기화 완료.** 주신 원문대로 두 사본의 `ERRCODE='40001'` 7곳을 `'PT409'` 로
치환했고, 경고 헤더는 동기화 기록으로 교체했습니다. 나머지 40001 둘(삭제 개체 수정 / 그룹에
개체 있음)도 바뀐 것 확인했습니다 — 감사합니다.
...
### 저희에게 답을 주실 것 — ✅ 2026-09-23 회신으로 전부 해소

1. 나머지 40001 둘 → PT409 로 바뀜 확인 (운영 DB: `redesign_save_pet_v1` 40001 0개 / PT409 3개)
2. SQL 원문 → 수령, 이 레포 사본 동기화 완료 (§6.2)
3. 9/7 문서 → 요지 수령 (앱 레포 git 이력). stall 방어와 시간대 다름 재확인, 별도 원인
