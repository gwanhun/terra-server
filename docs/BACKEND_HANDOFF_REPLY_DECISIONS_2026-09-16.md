...
## 추가 (2026-09-16 저녁) — skipped 발송 신호 수신, 반영

"`device.action.skipped` 발송 시작해도 됩니다"(앱 0.108.5+270) 받았습니다.

- 서버 스위치를 **켰습니다** (기본값을 켜짐으로 변경, 배포분에 포함). `guard` 객체는 항상 싣습니다 — `{ kind, metric, threshold, value }`.
- secret 은 기존 3종과 같은 값을 쓴다고 하셔서 별도 처리 없이 **한 값으로 4종이 나갑니다.**
- 따라서 secret 이 들어오는 순간 started / ended / failed / skipped 가 함께 활성화됩니다. 스테이징 4종 검증은 그때 같이 하시죠.
- 앱측 장애 등으로 skipped 만 잠시 끊어야 하면 저희 쪽 환경변수 하나로 즉시 차단됩니다. 말씀만 주세요.

기기 해제 실검증은 버려도 되는 기기가 생기면 하신다는 것, 알겠습니다. SQL 초안 검토는 별도 회신으로 드립니다.
