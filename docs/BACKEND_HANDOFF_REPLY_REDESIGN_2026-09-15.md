...
### 4.0 ⚠️ 정정 (2026-09-16, 앱 회신 §1-6 반영)

아래 §4.1 의 "없음" 판정은 **terra-server 저장소(코드·마이그레이션·문서) 기준**이며 그 범위에서는 정확합니다.
다만 **같은 Supabase 프로젝트의 `public` 스키마에는 앱 팀이 만든 `pets` 테이블과
`assign_pet_to_enclosure` RPC 가 이미 존재하고 현재 앱이 사용 중**입니다 (앱 팀 2026-09-16 확인).

**→ terra-server 는 `pets` 를 새로 만들거나 덮어쓰지 않습니다.** 마이그레이션에 추가하지 않고,
앱 팀 소유 객체로 취급합니다. (`behavior_logs` / `behavior_labels` 를 petcam-lab 소유로 두는 것과 동일한 방식)

이 정정은 §4.3·§4.4 의 결론을 바꾸지 않습니다 — 트랜잭션 수단이 서버에 없고, 관계 변경 이력은
DB 트리거로 보장하는 게 맞다는 판단은 그대로입니다.
